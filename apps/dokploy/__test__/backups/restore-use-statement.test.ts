...
const filter = (input: string) =>
	execFileSync(bash as string, ["-c", stripDatabaseSwitchCommand], {
		input,
	}).toString();

describe.skipIf(!bash)(
	"restore drops database-switch statements (mysql/mariadb)",
	() => {
		const dump = [
			"-- MariaDB dump",
		...
		it("is wired into mysql and mariadb restore pipelines only", () => {
			...
			for (const type of ["mysql", "mariadb"] as const) {
				...
				expect(cmd).toContain(
					`gunzip | ${stripDatabaseSwitchCommand} | docker`,
				);
			}
