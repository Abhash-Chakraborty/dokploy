...
import { KEEP_DOCKER_ENV } from "../process/docker-env";
...
Compose Type: ${composeType} ✅`;
	...
	set -e
	{
		...
		env -i PATH="$PATH" HOME="$HOME" ${KEEP_DOCKER_ENV} ${exportEnvCommand} docker ${command.split(" ").join(" ")} 2>&1 || { echo "Error: ❌ Docker command failed"; exit 1; }
